...
```
Another was of writing. Gives the same answer but might be easier to understand, can skip counting on our own.
```{r}
...
```{r}
diff_table_norm = diff_table
for (i in 1:3){
  diff_table_norm[,i] = diff_table[,i]/colSums(count_sums[i])
}
diff_table_norm
```
