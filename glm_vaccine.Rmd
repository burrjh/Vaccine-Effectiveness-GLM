...
ggplot( data = df, aes( date, rollmean(count,8,na.pad=TRUE), vaccinated )) + geom_line() 
...
ggplot( data = df_deaths, aes( date, rollmean(count,15,na.pad=TRUE), vaccinated )) + geom_line() 
```
...
df$critical = as.numeric(! df$event=="Cas")
...
```
```{r}
logReg = glm(critical ~ age + vaccinated, data=df_enrolled, family=binomial(link='logit'))
